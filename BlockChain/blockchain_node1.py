...
class Blockchain:
    ...
    def generate_genesis_block(self):
       """First block has index 0. It creates with func generate_genesis_block when the new chain is created"""
    ...
    def create_wallet(self):
        ...
        blockchain.writetocreatewallet(public_key,private_key,balance);
...
#--------------------------------------------------------------------------------------------------------
    @staticmethod
    def writetocreatewallet(public_key,private_key,balance):
       try:
           connection = MySQLdb.Connection(host='localhost', user='root', passwd='12345678', db='blockchain')
           cursor = connection.cursor()
          
           sql = """INSERT INTO wallet (public_key,private_key,balance) VALUES (%s,'%s',"%s")""" % (public_key,private_key,balance)
    ...
    @staticmethod
    def writetodb(sender, receiver, amount):
       try:
           connection = MySQLdb.Connection(host='localhost', user='root', passwd='12345678', db='blockchain')
           cursor = connection.cursor()
          
           sql = """INSERT INTO transaction (sender,receiver,amount,time) VALUES (%s,%s,'%s',"%s")""" % (sender,receiver,amount,time.strftime('%Y-%m-%d %H:%M:%S'))
           #sql = """INSERT INTO transaction (block_index,proof,previous_hash,transactions) VALUES (%d,%d,'%s',"%s")""" % (index,proof,previous_hash,transactions)
           cursor.execute(sql)
           connection.commit()
       except Exception as e:
    ...
    @staticmethod
    def writetomineblockdb(index,proof,previous_hash,transactions,merkle_root):
        try:
            connection = MySQLdb.Connection(host='localhost', user='root', passwd='12345678', db='blockchain')
            cursor = connection.cursor()
           
            sql = """INSERT INTO mine_block (index,proof,previous_hash,transactions,merkle_root,timestamp) VALUES (%s,%s,'%s','%s','%s',"%s")""" % (index,proof,previous_hash,transactions,merkle_root,time.strftime('%Y-%m-%d %H:%M:%S'))
            #sql = """INSERT INTO transaction (block_index,proof,previous_hash,transactions) VALUES (%d,%d,'%s',"%s")""" % (index,proof,previous_hash,transactions)
            cursor.execute(sql)
            connection.commit()
        except Exception as e:
    ...
    def create_block(self, proof, previous_hash, merkle_root):
        ...
        blockchain.writetomineblockdb(len(self.chain) + 1,proof,previous_hash,self.transactions,merkle_root)
        return block
...
def create_transaction():
    ...
    blockchain.writetodb(transaction['sender'],transaction['receiver'],transaction['amount']);
